...
def _run_background_tasks(background_tasks: BackgroundTasks) -> None:
    ...
    while the same chat.* patches are still active, since several of
    them (save_memory, update_mood, save_goal, complete_goal,
    maybe_track_interest) now happen inside that deferred task
    rather than inline."""
