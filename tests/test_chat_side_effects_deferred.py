...
# (output moderation logging, mood/goal/interest extraction,
# reminder/event detection) are genuinely DEFERRED to a background
# task rather than run inline before the reply is returned -- see
# chat._process_message_side_effects and the comment on
# chat._process_chat_message's background_tasks.add_task call.
#
# Input moderation is the one exception and deliberately stays in
# the blocking path: chat._is_crisis_message needs its categories
# synchronously, before the reply is returned, so that one call
# can't be deferred like the rest -- only its audit-trail logging
# (_flag_moderation) is. See the comment above the
# `input_moderation = moderate_text(message)` line in chat.py.
#
# This is the actual fix for messages piling up several sequential
...
# everything inline -- these specifically check what has and hasn't
# run yet at the moment _process_chat_message returns.
# ============================================================
...
def test_slow_side_effects_are_scheduled_not_run_inline():
    ...
    try:
        ...
        # The reply is already back. Input moderation already ran --
        # it's the one call that stays blocking, since the crisis
        # backstop needs its categories synchronously -- but nothing
        # else slow has run yet.
        assert result["reply"] == "hey!"
        assert mocks["moderate_text"].call_count == 1  # input only, so far
