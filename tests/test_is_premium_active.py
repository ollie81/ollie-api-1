...
def test_expired_web_subscription_is_not_premium_and_never_calls_play():
    ...
    # against, so a locally-expired one is trusted as genuinely
    # expired, with no _get_play_service call attempted at all. The
    # exact source value here is arbitrary -- the branch checks for
    # anything other than "play", not a specific provider name.
    with patch("premium.supabase") as mock_supabase, \
         patch("premium._get_play_service") as mock_get_service:
        _patch_subscription_row(mock_supabase, {
            "id": "sub-1", "status": "active", "expiry_time_millis": 1000,
            "product_id": "ollie_premium_monthly_web", "purchase_token": "web_txn_1",
            "source": "web",
        })
