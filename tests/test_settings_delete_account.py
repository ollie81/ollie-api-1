...
def test_wrong_confirmation_is_rejected_with_no_db_call():
    with patch("settings.OllieDB") as mock_db_cls:
        with pytest.raises(HTTPException) as exc_info:
            request_delete_account(DeleteAccountRequest(confirmation="delete"), _fake_request(), current_user={"id": "user-1"})
...
def test_blank_confirmation_is_rejected():
    with pytest.raises(HTTPException) as exc_info:
        request_delete_account(DeleteAccountRequest(confirmation=""), _fake_request(), current_user={"id": "user-1"})
    assert exc_info.value.status_code == 400
...
def test_correct_confirmation_schedules_deletion():
    with patch("settings.OllieDB") as mock_db_cls:
        ...
        result = request_delete_account(DeleteAccountRequest(confirmation="DELETE"), _fake_request(), current_user={"id": "user-1"})
...
def test_lowercase_confirmation_is_rejected():
    with pytest.raises(HTTPException) as exc_info:
        request_delete_account(DeleteAccountRequest(confirmation="delete my account"), _fake_request(), current_user={"id": "user-1"})
    assert exc_info.value.status_code == 400
