...
def google_login(req: GoogleAuthRequest, request: Request):
    try:
        ...
        # Google hands over an id_token, never a birthdate. Signup is
        # never blocked waiting on one -- same "absence never blocks"
        # rule email/phone signup already follow -- but a date IS
        # still validated if the client happens to send one.
        if is_new_user and req.date_of_birth:
            age_gate_error = _check_age_gate(req.date_of_birth)
            if age_gate_error:
                raise HTTPException(status_code=400, detail=age_gate_error)
